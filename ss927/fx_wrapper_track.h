...
// 外部 PTZ 只读 blind 状态。Tracker 不控制云台；该状态只标记当前图像坐标
...
// 销毁跟踪器实例（NULL 安全）。ACL global runtime 保持到进程退出，
// 因此同一进程可随后再次 track_create/track_init。
...
typedef struct {
    ...
    float zoomValue;           // 保持与 track-ptz legacy ABI 一致
