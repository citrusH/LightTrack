...
struct GStat {
    int    calls = 0;
    double ms    = 0.0;   // 本帧该 GMC 阶段累计耗时（★也用于 match 模块）
...
public:
    ...
    // ★ 新增：match 模块计时
    void add_match(double ms) { match_.calls++; match_.ms += ms; }
    ...
    void report() {
        ...
        // ★ 新增：match 模块耗时（独立打印，不计入模型总时间）
        if (match_.calls > 0) {
            std::printf("[ModelProfiler]   Match module  %4d            %8.2fms\n",
                        match_.calls, match_.ms);
        }
        ...
        if (any_gmc) {
            ...
            for (int i = 0; i < (int)GmcStage::COUNT; ++i) {
                ...
                if ((GmcStage)i != GmcStage::Total) gsum += g.ms;
...
// ★ 新增：match 模块专用计时探针，用法与 ScopedGmc 类似，无需指定阶段
class ScopedMatch {
public:
    ScopedMatch() : t0_(std::chrono::steady_clock::now()) {}
    ~ScopedMatch() {
        if (!kEnabled) return;
        auto t1 = std::chrono::steady_clock::now();
        double ms = std::chrono::duration<double, std::milli>(t1 - t0_).count();
        Profiler::instance().add_match(ms);
    }
    ScopedMatch(const ScopedMatch&) = delete;
    ScopedMatch& operator=(const ScopedMatch&) = delete;
private:
    std::chrono::steady_clock::time_point t0_;
};
