...
Result Utils::ReadBinFile(const std::string& fileName, void*& inputBuff, uint32_t& fileSize)
{
	...
	if (inputBuff == nullptr) {
		ERROR_LOG("malloc binFileBufferData failed, binFileBufferLen is %u", binFileBufferLen);
		binFile.close();
		return FAILED;
	}
...
Result Utils::MemcpyFileToDeviceBuffer(const std::string& fileName, void*& picDevBuffer, size_t inputBuffSize)
{
	...
	aclError aclRet = aclrtMemcpy(picDevBuffer, inputBuffSize, inputBuff,
		inputBuffSize, ACL_MEMCPY_HOST_TO_DEVICE);
	std::free(inputBuff);
	if (aclRet != ACL_SUCCESS) {
		ERROR_LOG("memcpy h2d failed. buffer size is %zu, errorCode is %d",
...
Result Utils::MemcpyFileToDeviceBuffer(unsigned char* img_data, void*& picDevBuffer, size_t inputBuffSize)
{
	if (img_data == nullptr || picDevBuffer == nullptr || inputBuffSize == 0) return FAILED;
	aclError ret = aclrtMemcpy(picDevBuffer, inputBuffSize, img_data,
		inputBuffSize, ACL_MEMCPY_HOST_TO_DEVICE);
	if (ret != ACL_SUCCESS) {
		ERROR_LOG("memcpy h2d failed. buffer size is %zu, errorCode is %d",
...
cv::Rect_<float> Utils::convert_z_to_bbox(const cv::Mat& z) {
    ...
    bbox.width = bbox.x + w;              // 右下
    bbox.height = bbox.y + h;             // 
