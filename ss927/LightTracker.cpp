...
int LightTracker::init() {
    ...
    if (acl_runtime_.init(0) != 0) {
        ERROR_LOG("tracker ACL runtime initialization failed");
        return -1;
    }

    std::string model_dir = "/oem/model";
    size_t sizeModel_reid = 0, sizeModel_faceR = 0, sizeModel_pd = 0, sizeModel_fkps = 0, sizeModel_pose = 0;
    uint8_t* bufferModel_reid = Utils::loadModel(
        (model_dir + "/ptz_e01_mobilenetv2_gem_512_opset11_allowfp32tofp16.om").c_str(),
        sizeModel_reid);
    uint8_t* bufferModel_face_rec = Utils::loadModel((model_dir + "/faceReco_v2.om").c_str(), sizeModel_faceR);
    uint8_t* bufferModel_det = Utils::loadModel(
        (model_dir + "/yolox_416_face_body_head.om").c_str(), sizeModel_pd);
    uint8_t* bufferModel_fkps = Utils::loadModel((model_dir + "/faceKps_v7_01.om").c_str(), sizeModel_fkps);
    uint8_t* bufferModel_pose = Utils::loadModel((model_dir + "/rtmpose-t.om").c_str(), sizeModel_pose);

    // A11 诊断防线：任一 .om 缺失/损坏 → 明确报错 + init 失败。
    if (!bufferModel_reid || !bufferModel_face_rec || !bufferModel_det
        || !bufferModel_fkps || !bufferModel_pose) {
        ERROR_LOG("model load failed under %s: reid=%zu faceReco=%zu det=%zu faceKps=%zu pose=%zu"
                  " (0 = missing/unreadable)", model_dir.c_str(),
                  sizeModel_reid, sizeModel_faceR, sizeModel_pd, sizeModel_fkps, sizeModel_pose);
        if (bufferModel_reid)     free(bufferModel_reid);
        if (bufferModel_face_rec) free(bufferModel_face_rec);
        if (bufferModel_det)      free(bufferModel_det);
        if (bufferModel_fkps)     free(bufferModel_fkps);
        if (bufferModel_pose)     free(bufferModel_pose);
        release_models();
        acl_runtime_.release();
        return -1;
    }

    Detector_yolox::Config config_pd;
    config_pd.sizeModel = sizeModel_pd;
    config_pd.bufferModel = bufferModel_det;
    // 当前 YOLOX OM/AIPP 契约使用 OpenCV BGR HWC U8 输入。
    config_pd.bgr2rgb = false;

    VISION_ENGINE::ReIDConfig config_reid;
    config_reid.sizeModel = sizeModel_reid;
    config_reid.bufferModel = bufferModel_reid;
    config_reid.inputSize = cv::Size(128, 256);
    config_reid.bgr2rgb = true;
    config_reid.deviceId = 0;

    VISION_ENGINE::FaceKeypointConfig config_fk;
    config_fk.sizeModel = sizeModel_fkps;
    config_fk.bufferModel = bufferModel_fkps;
    config_fk.inputSize = cv::Size(192, 192);
    config_fk.bgr2rgb = true;
    config_fk.deviceId = 0;
    config_fk.numPoints = 106;

    VISION_ENGINE::FaceRecogConfig config_fr;
    config_fr.sizeModel = sizeModel_faceR;
    config_fr.bufferModel = bufferModel_face_rec;
    config_fr.inputSize = cv::Size(112, 112);
    config_fr.bgr2rgb = true;
    config_fr.deviceId = 0;
    config_fr.imageSize = 112;
    config_fr.find_sub = false;
    config_fr.find_mul = false;

    VISION_ENGINE::PoseEstimatorConfig config_pose;
    config_pose.sizeModel    = sizeModel_pose;
    config_pose.bufferModel  = bufferModel_pose;
    config_pose.bgr2rgb      = true;
    config_pose.deviceId     = 0;
    config_pose.kpConfThresh = 0.4f;
    // 按成员声明顺序初始化，正常析构与失败回滚都严格逆序释放。
    int rc_fk = fail_model_init_for_test("face_kps")
              ? -3 : detector_fk.init(config_fk);
    int rc_fr = (rc_fk == 0)
              ? (fail_model_init_for_test("face_reco")
                 ? -3 : face_recognizer.init(config_fr, 0.50f, 0.40f)) : -2;
    int rc_reid = (rc_fr == 0)
                ? (fail_model_init_for_test("reid")
                   ? -3 : emb_model.init(config_reid)) : -2;
    int rc_det = (rc_reid == 0)
               ? (fail_model_init_for_test("detector")
                  ? -3 : detector.init(config_pd)) : -2;
    int rc_pose = (rc_det == 0)
                ? (fail_model_init_for_test("pose")
                   ? -3 : pose_estimator.init(config_pose)) : -2;
    ...
    // A12 诊断防线：任一组件 ACL/模型 init 失败 → 明确报错 + init 失败。
    if (rc_det != 0 || rc_reid != 0 || rc_fk != 0 || rc_fr != 0 || rc_pose != 0) {
        ERROR_LOG("component init failed: det=%d reid=%d faceKps=%d faceReco=%d pose=%d"
                  " (-2 = not attempted, -3 = injected, other nonzero = component failure)",
                  rc_det, rc_reid, rc_fk, rc_fr, rc_pose);
        release_models();
        acl_runtime_.release();
        return -1;
    }
...
std::pair<cv::Mat, int> LightTracker::setMainTarget(const cv::Mat& img, const cv::Rect& mainBox) {
    ...
    // 注册框已由上层确认；Pose 只刷新 committed 状态，不参与本次身份选择。
    {
        ...
        PoseCacheEntry* entry = request_pose(
            img, bbox_for_match, -1, -1, PoseReason::FINAL_BODY_OBSERVATION);
        if (entry && entry->status == PoseRequestStatus::AVAILABLE) {
            observe_final_body_pose(entry, bbox_for_match, img, -1, -1);
...
bool LightTracker::compute_ptz_trunk_box(const cv::Mat& body_box,
                                         const PoseResult& pose,
                                         cv::Mat& trunk_box)
{
    trunk_box.release();
    const char* fallback = "NONE";
    float shoulder_width = -1.f;
    float estimated_top = -1.f;
    bool hand_raised = false;
    const PoseKeypoint& left_shoulder = pose.keypoints[LEFT_SHOULDER];
    const PoseKeypoint& right_shoulder = pose.keypoints[RIGHT_SHOULDER];
    const PoseKeypoint& left_wrist = pose.keypoints[LEFT_WRIST];
    const PoseKeypoint& right_wrist = pose.keypoints[RIGHT_WRIST];

    auto trace = [&](const char* action, const cv::Mat& out) {
        if (!kMatchTrace) return;
        const bool body_valid = !body_box.empty() && body_box.cols >= 4;
        const bool out_valid = !out.empty() && out.cols >= 4;
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[PTZ_TRUNK] f=%d action=%s body=(%.0f,%.0f,%.0f,%.0f)"
            " trunk=(%.0f,%.0f,%.0f,%.0f) shoulder_w=%.1f"
            " ls=%.2f rs=%.2f lw=%.2f rw=%.2f raised=%d estimated_top=%.1f fallback=%s",
            frame_count, action,
            body_valid ? body_box.at<float>(0, 0) : -1.f,
            body_valid ? body_box.at<float>(0, 1) : -1.f,
            body_valid ? body_box.at<float>(0, 2) : -1.f,
            body_valid ? body_box.at<float>(0, 3) : -1.f,
            out_valid ? out.at<float>(0, 0) : -1.f,
            out_valid ? out.at<float>(0, 1) : -1.f,
            out_valid ? out.at<float>(0, 2) : -1.f,
            out_valid ? out.at<float>(0, 3) : -1.f,
            shoulder_width,
            left_shoulder.confidence, right_shoulder.confidence,
            left_wrist.confidence, right_wrist.confidence,
            hand_raised ? 1 : 0, estimated_top, fallback);
        trace_push(line);
    };

    if (body_box.empty() || body_box.cols < 4 || img_w <= 1 || img_h <= 1) {
        fallback = "INVALID_BODY_OR_FRAME";
        trace("fallback", body_box);
        return false;
    }
    const float body_x1 = body_box.at<float>(0, 0);
    const float body_y1 = body_box.at<float>(0, 1);
    const float body_x2 = body_box.at<float>(0, 2);
    const float body_y2 = body_box.at<float>(0, 3);
    if (!std::isfinite(body_x1) || !std::isfinite(body_y1)
        || !std::isfinite(body_x2) || !std::isfinite(body_y2)
        || body_x2 <= body_x1 || body_y2 <= body_y1) {
        fallback = "INVALID_BODY_BOX";
        trace("fallback", body_box);
        return false;
    }
    if (left_shoulder.confidence < kPtzTrunkKeypointConf
        || right_shoulder.confidence < kPtzTrunkKeypointConf
        || !std::isfinite(left_shoulder.x) || !std::isfinite(left_shoulder.y)
        || !std::isfinite(right_shoulder.x) || !std::isfinite(right_shoulder.y)) {
        fallback = "SHOULDER_INVALID";
        trace("fallback", body_box);
        return false;
    }

    const float shoulder_x1 = std::min(left_shoulder.x, right_shoulder.x);
    const float shoulder_x2 = std::max(left_shoulder.x, right_shoulder.x);
    shoulder_width = shoulder_x2 - shoulder_x1;
    if (!std::isfinite(shoulder_width)
        || shoulder_width < kPtzTrunkMinShoulderWidthPx) {
        fallback = "SHOULDER_WIDTH_INVALID";
        trace("fallback", body_box);
        return false;
    }

    const float margin = std::max(3.f,
        shoulder_width * kPtzTrunkHandRaiseMarginRatio);
    const bool left_raised = left_wrist.confidence >= kPtzTrunkKeypointConf
                          && std::isfinite(left_wrist.y)
                          && left_wrist.y < left_shoulder.y - margin;
    const bool right_raised = right_wrist.confidence >= kPtzTrunkKeypointConf
                           && std::isfinite(right_wrist.y)
                           && right_wrist.y < right_shoulder.y - margin;
    hand_raised = left_raised || right_raised;

    const float horizontal_pad = shoulder_width * kPtzTrunkShoulderSidePadRatio;
    float trunk_x1 = shoulder_x1 - horizontal_pad;
    float trunk_x2 = shoulder_x2 + horizontal_pad;
    float trunk_y1 = body_y1;
    float trunk_y2 = body_y2;
    const float shoulder_center_y = (left_shoulder.y + right_shoulder.y) * 0.5f;
    estimated_top = shoulder_center_y - shoulder_width * kPtzTrunkHeadTopRatio;
    if (hand_raised)
        trunk_y1 = estimated_top;

    const float max_x = (float)img_w - 1.f;
    const float max_y = (float)img_h - 1.f;
    trunk_x1 = std::max(0.f, std::min(trunk_x1, max_x));
    trunk_x2 = std::max(0.f, std::min(trunk_x2, max_x));
    trunk_y1 = std::max(0.f, std::min(trunk_y1, max_y));
    trunk_y2 = std::max(0.f, std::min(trunk_y2, max_y));
    estimated_top = std::max(0.f, std::min(estimated_top, max_y));
    if (!std::isfinite(trunk_x1) || !std::isfinite(trunk_y1)
        || !std::isfinite(trunk_x2) || !std::isfinite(trunk_y2)
        || trunk_x2 <= trunk_x1 || trunk_y2 <= trunk_y1) {
        fallback = "TRUNK_INVALID";
        trace("fallback", body_box);
        return false;
    }

    trunk_box = (cv::Mat_<float>(1, 4) << trunk_x1, trunk_y1, trunk_x2, trunk_y2);
    trace("apply", trunk_box);
    return true;
}

void LightTracker::reset_ptz_body_output_transition()
{
    ptz_body_output_transition_.last_trunk_valid = false;
    ptz_body_output_transition_.last_trunk_box.release();
    ptz_body_output_transition_.last_trunk_frame = -1;
    ptz_body_output_transition_.last_output_valid = false;
    ptz_body_output_transition_.last_output_box.release();
    ptz_body_output_transition_.last_output_frame = -1;
}

cv::Mat LightTracker::apply_ptz_body_output_transition(
    const cv::Mat& raw_body_box, const cv::Mat& current_trunk_box)
{
    if (raw_body_box.empty() || raw_body_box.cols < 4) return cv::Mat();

    const bool trunk_valid = !current_trunk_box.empty()
                          && current_trunk_box.cols >= 4;
    const int trunk_age = ptz_body_output_transition_.last_trunk_valid
                       ? frame_count - ptz_body_output_transition_.last_trunk_frame
                       : -1;
    const char* action = "RAW_BLEND";
    cv::Mat target = raw_body_box;
    bool exact_hold = false;

    if (trunk_valid) {
        target = current_trunk_box;
        ptz_body_output_transition_.last_trunk_valid = true;
        ptz_body_output_transition_.last_trunk_box = current_trunk_box.clone();
        ptz_body_output_transition_.last_trunk_frame = frame_count;
        action = "TRUNK_BLEND";
    } else if (ptz_body_output_transition_.last_trunk_valid
               && trunk_age >= 0 && trunk_age <= kPtzTrunkHoldFrames) {
        target = ptz_body_output_transition_.last_trunk_box;
        action = "TRUNK_HOLD";
        exact_hold = true;
    }

    cv::Mat out = target.clone();
    const bool can_blend = !exact_hold
                        && ptz_body_output_transition_.last_output_valid
                        && ptz_body_output_transition_.last_output_frame == frame_count - 1
                        && !ptz_body_output_transition_.last_output_box.empty()
                        && ptz_body_output_transition_.last_output_box.cols >= 4;
    if (can_blend) {
        const cv::Mat& prev = ptz_body_output_transition_.last_output_box;
        const float a = kPtzTrunkTransitionAlpha;
        out = (cv::Mat_<float>(1, 4) <<
            prev.at<float>(0, 0) + a * (target.at<float>(0, 0) - prev.at<float>(0, 0)),
            prev.at<float>(0, 1) + a * (target.at<float>(0, 1) - prev.at<float>(0, 1)),
            prev.at<float>(0, 2) + a * (target.at<float>(0, 2) - prev.at<float>(0, 2)),
            prev.at<float>(0, 3) + a * (target.at<float>(0, 3) - prev.at<float>(0, 3)));
    }
    cv::Mat clipped = clip_reconstructed_body_to_frame(out, "PTZ_TRUNK_TRANSITION");
    if (!clipped.empty()) out = clipped;

    ptz_body_output_transition_.last_output_valid = !out.empty();
    ptz_body_output_transition_.last_output_box = out.clone();
    ptz_body_output_transition_.last_output_frame = frame_count;

    if (kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[PTZ_TRUNK_TRANSITION] f=%d action=%s trunk_valid=%d age=%d blend=%d"
            " raw=(%.0f,%.0f,%.0f,%.0f) target=(%.0f,%.0f,%.0f,%.0f)"
            " out=(%.0f,%.0f,%.0f,%.0f)",
            frame_count, action, trunk_valid ? 1 : 0, trunk_age, can_blend ? 1 : 0,
            raw_body_box.at<float>(0, 0), raw_body_box.at<float>(0, 1),
            raw_body_box.at<float>(0, 2), raw_body_box.at<float>(0, 3),
            target.at<float>(0, 0), target.at<float>(0, 1),
            target.at<float>(0, 2), target.at<float>(0, 3),
            out.empty() ? -1.f : out.at<float>(0, 0),
            out.empty() ? -1.f : out.at<float>(0, 1),
            out.empty() ? -1.f : out.at<float>(0, 2),
            out.empty() ? -1.f : out.at<float>(0, 3));
        trace_push(line);
    }
    return out;
}
...
bool LightTracker::face_owner_allows_non_main(
    int face_idx, std::string* reason) const
{
    if (face_idx < 0 || face_idx >= (int)recovery_faces_.size()) {
        if (reason) *reason = "face_not_current";
        return false;
    }
    if (face_idx >= (int)recovery_face_owner_person_.size()
        || recovery_face_owner_person_[face_idx] < 0) {
        if (reason) *reason = "no_owner";
        return false;
    }
    if (face_idx < (int)recovery_face_owner_ambiguous_.size()
        && recovery_face_owner_ambiguous_[face_idx]) {
        if (reason) *reason = "owner_ambiguous";
        return false;
    }
    if (face_idx >= (int)recovery_face_owner_exclusion_eligible_.size()
        || !recovery_face_owner_exclusion_eligible_[face_idx]) {
        if (reason) *reason = "not_official_owner_face";
        return false;
    }
    const int owner = recovery_face_owner_person_[face_idx];
    if (owner >= (int)recovery_body_boxes_.size()
        || owner >= (int)recovery_body_valid_.size()
        || !recovery_body_valid_[owner]) {
        if (reason) *reason = "owner_not_valid";
        return false;
    }
    if (reason) *reason = "unique_official_owner";
    return true;
}

void LightTracker::promote_reliable_face_non_main(const cv::Rect& face_box)
{
    int face_idx = -1;
    for (int i = 0; i < (int)recovery_faces_.size(); ++i) {
        if (recovery_faces_[i].x == face_box.x && recovery_faces_[i].y == face_box.y
            && recovery_faces_[i].width == face_box.width
            && recovery_faces_[i].height == face_box.height) {
            face_idx = i;
            break;
        }
    }
    FaceInferenceCacheEntry* cached_match = nullptr;
    for (auto& cached : face_inference_cache_) {
        if (cached.box.x == face_box.x && cached.box.y == face_box.y
            && cached.box.width == face_box.width && cached.box.height == face_box.height) {
            cached_match = &cached;
            break;
        }
    }
    std::string gate;
    if (cached_match == nullptr || !cached_match->comparison_completed) {
        gate = "comparison_not_completed";
    } else if (cached_match->quality < kFaceReliableNonMainMinQ) {
        gate = "quality_below_non_main";
    } else if (!std::isfinite(cached_match->sim)
               || cached_match->sim > kFaceReliableNonMainMaxSimilarity) {
        gate = "similarity_not_low_enough";
    }
    if (!gate.empty()) {
        if (kMatchTrace) {
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[FACE_NON_MAIN] f=%d action=unknown face=%d sim=%.2f max=%.2f"
                " q=%.2f minQ=%.2f gate=%s",
                frame_count, face_idx,
                cached_match ? cached_match->sim : -1.f,
                kFaceReliableNonMainMaxSimilarity,
                cached_match ? cached_match->quality : -1.f,
                kFaceReliableNonMainMinQ, gate.c_str());
            trace_push(line);
        }
        return;
    }
    if (!face_owner_allows_non_main(face_idx, &gate)) {
        if (kMatchTrace) {
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[FACE_NON_MAIN] f=%d action=unknown face=%d sim=%.2f max=%.2f"
                " q=%.2f minQ=%.2f gate=%s",
                frame_count, face_idx, cached_match->sim,
                kFaceReliableNonMainMaxSimilarity, cached_match->quality,
                kFaceReliableNonMainMinQ, gate.c_str());
            trace_push(line);
        }
        return;
    ...
    const int owner = recovery_face_owner_person_[face_idx];
    cached_match->reliable_non_main = true;
    cached_match->evidence = IdentityEvidence::NEGATIVE;
    cached_match->reason = "reliable_face_non_main";
    const cv::Rect& body = recovery_body_boxes_[owner];
    cv::Mat body_box = (cv::Mat_<float>(1, 4) <<
        (float)body.x, (float)body.y, (float)body.width, (float)body.height);
    mark_body_identity_evidence(body_box, IdentityEvidence::NEGATIVE,
                                "reliable_face_non_main");
    if (kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[FACE_NON_MAIN] f=%d action=exclude face=%d owner=%d sim=%.2f max=%.2f"
            " q=%.2f minQ=%.2f gate=%s",
            frame_count, face_idx, owner, cached_match->sim,
            kFaceReliableNonMainMaxSimilarity, cached_match->quality,
            kFaceReliableNonMainMinQ, gate.c_str());
        trace_push(line);
        trace_event_pending_ = true;
    ...
    int owner_person, std::string* reason) const
{
    ...
    if (evidence != IdentityEvidence::POSITIVE) {
        for (int i = 0; i < (int)recovery_faces_.size(); ++i) {
            ...
            bool reliable_non_main = false;
            for (const auto& cached : face_inference_cache_) {
                if (cached.box.x == recovery_faces_[i].x
                    && cached.box.y == recovery_faces_[i].y
                    && cached.box.width == recovery_faces_[i].width
                    && cached.box.height == recovery_faces_[i].height) {
                    reliable_non_main = cached.reliable_non_main;
                    break;
                }
            }
            if (face_evidence == IdentityEvidence::NEGATIVE && reliable_non_main) {
                evidence = IdentityEvidence::NEGATIVE;
                why = "reliable_face_non_main";
                break;
            }
        }
    }
...
{
    const int64_t now = current_frame_timestamp_ms_ >= 0
                      ? current_frame_timestamp_ms_ : now_ms_const();
    if (emergence_valid_ && now >= emergence_update_ms_
        && now - emergence_update_ms_ <= kEmergenceMaxAgeMs) {
        cx = emergence_cx_;
        cy = emergence_cy_;
        return true;
    }
...
bool LightTracker::coast_search_hint_center(float& cx, float& cy,
                                            int64_t* age_ms) const
{
    if (!coast_search_hint_.valid) return false;
    const int64_t now = current_frame_timestamp_ms_ >= 0
                      ? current_frame_timestamp_ms_ : now_ms_const();
    const int64_t age = now - coast_search_hint_.update_ms;
    if (age < 0 || age > kCoastSearchHintHoldMs
        || !std::isfinite(coast_search_hint_.cx)
        || !std::isfinite(coast_search_hint_.cy)) {
        return false;
    }
    cx = coast_search_hint_.cx;
    cy = coast_search_hint_.cy;
    if (age_ms) *age_ms = age;
    return true;
}

void LightTracker::update_coast_search_hint(const cv::Mat& predicted_box,
                                            int64_t now)
{
    if (predicted_box.empty() || predicted_box.cols < 4) return;
    const float x1 = predicted_box.at<float>(0, 0);
    const float y1 = predicted_box.at<float>(0, 1);
    const float x2 = predicted_box.at<float>(0, 2);
    const float y2 = predicted_box.at<float>(0, 3);
    if (!std::isfinite(x1) || !std::isfinite(y1)
        || !std::isfinite(x2) || !std::isfinite(y2)
        || x2 <= x1 || y2 <= y1) {
        return;
    }
    coast_search_hint_.valid = true;
    coast_search_hint_.cx = 0.5f * (x1 + x2);
    coast_search_hint_.cy = 0.5f * (y1 + y2);
    coast_search_hint_.update_ms = now;
    coast_search_hint_.anchor_frame_id = frozen_prediction_.anchor_frame_id;
    coast_search_hint_.mode = frozen_prediction_.mode;
    if (kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[COAST_SEARCH_HINT] f=%d action=update center=(%.1f,%.1f)"
            " anchor_f=%d mode=%s permission=search_rank_only",
            frame_count, coast_search_hint_.cx, coast_search_hint_.cy,
            coast_search_hint_.anchor_frame_id,
            prediction_mode_name(coast_search_hint_.mode));
        trace_push(line);
    }
}

void LightTracker::clear_coast_search_hint(const char* reason)
{
    if (coast_search_hint_.valid && kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[COAST_SEARCH_HINT] f=%d action=clear reason=%s center=(%.1f,%.1f)"
            " anchor_f=%d",
            frame_count, reason ? reason : "unknown",
            coast_search_hint_.cx, coast_search_hint_.cy,
            coast_search_hint_.anchor_frame_id);
        trace_push(line);
        trace_event_pending_ = true;
    }
    coast_search_hint_ = CoastSearchHint{};
}

void LightTracker::expire_coast_search_hint(int64_t now)
{
    if (!coast_search_hint_.valid) return;
    const int64_t age = now - coast_search_hint_.update_ms;
    if (age >= 0 && age <= kCoastSearchHintHoldMs) return;
    if (kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[COAST_SEARCH_HINT] f=%d action=expire age=%lldms max=%lldms"
            " anchor_f=%d",
            frame_count, (long long)std::max<int64_t>(0, age),
            (long long)kCoastSearchHintHoldMs,
            coast_search_hint_.anchor_frame_id);
        trace_push(line);
        trace_event_pending_ = true;
    }
...
void LightTracker::clear_short_prediction(const char* reason)
{
    ...
    clear_coast_search_hint(reason);
}
...
bool LightTracker::try_short_prediction(cv::Mat& out_box, const char* context)
{
    ...
    if (frozen_prediction_.lifecycle == PredictionLifecycle::IDLE) {
        ...
        const float body_width = x2 - x1;
        const float frame_diag = std::hypot((float)img_w, (float)img_h);
        frozen.max_body_width_displacement =
            kShortPredictionMaxBodyWidths * body_width;
        frozen.max_frame_displacement =
        ...
        if (kMatchTrace) {
            ...
            std::snprintf(line, sizeof(line),
                ...
                " frozen_v=(%.3f,%.3f)pxms max_ms=%lld"
                " width_cap=%.1f(%.2fx%.1f) frame_cap=%.1f max_disp=%.1f"
    ...
    if (!std::isfinite(displacement)
        ...
        if (kMatchTrace) {
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[SHORT_PRED_LIMIT] f=%d action=reject reason=max_displacement"
                " predicted_disp=%.1f max_disp=%.1f width_cap=%.1f"
                " frame_cap=%.1f motion_age=%lldms",
                frame_count, displacement, frozen_prediction_.max_displacement,
                frozen_prediction_.max_body_width_displacement,
                frozen_prediction_.max_frame_displacement,
    ...
    // 最终提示必须使用整框边界修正后的画面位置，不能用越界前的 raw next_cx/cy。
    frozen_prediction_.last_output_cx = 0.5f * (x1 + x2);
    frozen_prediction_.last_output_cy = 0.5f * (y1 + y2);
    frozen_prediction_.last_output_ms = now;
    update_coast_search_hint(out_box, now);
...
void LightTracker::set_ptz_blind_phase(int phase) {
    ...
    if (next != PtzBlindPhase::IDLE) {
        ...
        // generic pending 用旧图像坐标中心复验，不能跨 PTZ 坐标系继续沿用。
        pending_active_ = false;
        pending_src_ = pending_idx_ = -1;
        pending_from_sweep_ = false;
        reacq_defer_count_ = 0;
        // Face global pending/hypothesis 同样携带旧画面位置，只能在新坐标系中
        // 从当前帧重新建立；Face 模板本身保留。
...
bool LightTracker::try_face_only_continuity(const cv::Mat& img, cv::Mat& out_box) {
    ...
    float coast_cx = -1.f, coast_cy = -1.f;
    int64_t coast_age_ms = -1;
    const bool have_coast_hint = coast_search_hint_center(
        coast_cx, coast_cy, &coast_age_ms);
    cv::Point2f coast_ref(coast_cx, coast_cy);
    if (have_coast_hint && kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[RECOVERY_SEARCH_REF] f=%d use=face_order primary=trusted"
            " secondary=coast_hint center=(%.1f,%.1f) age=%lldms"
            " permission=search_rank_only",
            frame_count, coast_cx, coast_cy, (long long)coast_age_ms);
        trace_push(line);
    }
    ...
    Verification_Result verification = face_recognition_verification(
        ...
        &ref, gate, budget, have_coast_hint ? &coast_ref : nullptr);
    ...
    const std::vector<ObjDetInfo>& detect_list)
{
    ...
    recovery_face_owner_exclusion_eligible_.clear();
    ...
    recovery_face_owner_exclusion_eligible_.assign(faces.size(), 0);
    ...
    for (size_t face_i = 0; face_i < faces.size(); ++face_i) {
        ...
        if (best_person != -1) {
            ...
            if (!choice.valid || geom_cost < choice.geom_cost - 1e-4f
                ...
                choice.valid = true;
                choice.raw_face_index = (int)face_i;
                choice.box = face.box;
    ...
    for (size_t i = 0; i < face_choice.size(); ++i) {
        ...
        const int fi = face_choice[i].raw_face_index;
        if (fi >= 0 && fi < (int)recovery_face_owner_exclusion_eligible_.size()
            && fi < (int)recovery_face_owner_ambiguous_.size()
            && !recovery_face_owner_ambiguous_[fi]
            && fi < (int)recovery_face_owner_person_.size()
            && recovery_face_owner_person_[fi] == (int)i) {
            recovery_face_owner_exclusion_eligible_[fi] = 1;
        }
    }
...
* img: frame
*/
std::pair<cv::Mat,int> LightTracker::update(const cv::Mat& img, const cv::Rect& mainBox) {
    if (!initialized_) {
    ...
    expire_coast_search_hint(ts_ms);
    ...
    pose_cache_.clear();
    pose_cache_.reserve(kPoseCandidateBudgetPerFrame + kPoseFinalBudgetPerFrame);
    pose_budget_used_ = 0;
    pose_candidate_budget_used_ = 0;
    pose_final_budget_used_ = 0;
    current_pose_observation_ = CurrentPoseObservation{};
    cv::Mat result;
	int person_cnt = 0;
    ...
    face_non_main_promotion_enabled_ = false;
    ...
    auto finalize_frame = [&](const cv::Mat& out_box) {
        ...
        // trunk/raw 过渡只属于最终可靠 BODY。HEAD/FACE/NONE 一旦成为输出来源，
        // 立即丢弃该 PTZ-only 缓存，避免跨来源把旧 trunk 当作当前人体几何。
        if (frame_output_source_ != OutputSource::BODY)
            reset_ptz_body_output_transition();
    ...
    if (no_body_detections) {
        if(trackers.size() != 0){
            ...
            // Tracker 不再输出 image-space PRED，避免两层同时驱动云台；同时旧预测
            // 搜索提示不能遗留到外部滑动后的新画面坐标系。
    ...
    if (main_result_row >= 0) {
        ...
        if (face_track_override) {
            ...
            // 既有稳定器只继续处理原始 BODY 输出语义；trunk/raw 的专用过渡在
            // 其后单独完成，绝不把 trunk 反写进 BODY/KF/lead/Motion History。
            cv::Mat stable_body_out = stabilize_returned_box(
                raw_body_out, OutputSource::BODY, false);
            if (stable_body_out.empty())
                stable_body_out = raw_body_out;
            cv::Mat trunk_box;
            const bool pose_matches_output_body = current_pose_observation_.valid
                && !current_pose_observation_.body_box.empty()
                && current_pose_observation_.body_box.cols >= 4
                && std::fabs(current_pose_observation_.body_box.at<float>(0, 0)
                             - raw_body_out.at<float>(0, 0)) <= 2.f
                && std::fabs(current_pose_observation_.body_box.at<float>(0, 1)
                             - raw_body_out.at<float>(0, 1)) <= 2.f
                && std::fabs(current_pose_observation_.body_box.at<float>(0, 2)
                             - raw_body_out.at<float>(0, 2)) <= 2.f
                && std::fabs(current_pose_observation_.body_box.at<float>(0, 3)
                             - raw_body_out.at<float>(0, 3)) <= 2.f;
            if (pose_matches_output_body
                && compute_ptz_trunk_box(raw_body_out,
                                         current_pose_observation_.pose, trunk_box)) {
                // trunk 有效时刷新 PTZ-only cache；过渡层保证不会与 raw BODY 瞬时跳变。
            } else if (kMatchTrace) {
                char line[kTraceLineLen];
                std::snprintf(line, sizeof(line),
                    "[PTZ_TRUNK] f=%d action=fallback body=(%.0f,%.0f,%.0f,%.0f)"
                    " trunk=(%.0f,%.0f,%.0f,%.0f) shoulder_w=-1.0"
                    " ls=-1.00 rs=-1.00 lw=-1.00 rw=-1.00 raised=0"
                    " estimated_top=-1.0 fallback=%s",
                    frame_count,
                    raw_body_out.at<float>(0, 0), raw_body_out.at<float>(0, 1),
                    raw_body_out.at<float>(0, 2), raw_body_out.at<float>(0, 3),
                    stable_body_out.empty() ? -1.f : stable_body_out.at<float>(0, 0),
                    stable_body_out.empty() ? -1.f : stable_body_out.at<float>(0, 1),
                    stable_body_out.empty() ? -1.f : stable_body_out.at<float>(0, 2),
                    stable_body_out.empty() ? -1.f : stable_body_out.at<float>(0, 3),
                    current_pose_observation_.valid
                        ? "POSE_BODY_MISMATCH" : "POSE_NOT_AVAILABLE");
                trace_push(line);
            }
            out_box = apply_ptz_body_output_transition(stable_body_out, trunk_box);
            for (int c = 0; c < 4; ++c)
    ...
    if (!got_main) {
        ...
        if (face_identity_hit || face_track_hit) {
            ...
            // 同上：BODY 候选存在但未安全确认时也不能输出 PRED。外部 PTZ 将
            // 收到无主框并按最后真实框进入/继续 blind slide；Tracker 只负责重捕。
            clear_short_prediction("external_ptz_blind");
            coast_weight_ = 0.f;
...
std::pair<float, std::string> LightTracker::face_recognition_inference(const cv::Mat& face_boxes, cv::Mat img) {
    ...
    for(int i = 0; i < face_boxes.rows; i++) {
        ...
        bool cached_completed = false;
        bool cached_non_main = false;
        for (const auto& cached : face_inference_cache_) {
            if (cached.box.x != box.x || cached.box.y != box.y
            ...
            cached_completed = cached.comparison_completed;
            cached_non_main = cached.reliable_non_main;
        ...
        if (cache_hit) {
            if (face_non_main_promotion_enabled_ && cached_name == "unknown") {
                for (const auto& cached : face_inference_cache_) {
                    if (cached.box.x == box.x && cached.box.y == box.y
                        && cached.box.width == box.width
                        && cached.box.height == box.height
                        && cached.comparison_completed) {
                        promote_reliable_face_non_main(box);
                        break;
                    }
                }
                for (const auto& cached : face_inference_cache_) {
                    if (cached.box.x != box.x || cached.box.y != box.y
                        || cached.box.width != box.width
                        || cached.box.height != box.height) continue;
                    cached_evidence = cached.evidence;
                    cached_reason = cached.reason;
                    cached_completed = cached.comparison_completed;
                    cached_non_main = cached.reliable_non_main;
            ...
            if (kMatchTrace) {
                ...
                std::snprintf(line, sizeof(line),
                    ...
                    " sim=%.2f name=%s completed=%d evidence=%s reliableNonMain=%d reason=%s",
                    frame_count, box.x, box.y, box.width, box.height,
                    cached_sim, cached_name.c_str(),
                    cached_completed ? 1 : 0,
                    identity_evidence_name(cached_evidence),
                    cached_non_main ? 1 : 0, cached_reason.c_str());
                trace_push(line);
            }
            continue;
        }
        ...
        const bool comparison_completed = recognition_result.comparison_completed;
        // 普通未命中 A 只是事实，不是主身份负证据。只有下方经过 owner
        // 安全审计后，才会升级为 reliable_face_non_main。
        IdentityEvidence evidence = comparison_completed
                                 && recognition_result.user_name != "unknown"
        ...
        FaceInferenceCacheEntry& cached = face_inference_cache_.back();
        cached.quality = face_q;
        cached.comparison_completed = comparison_completed;
        if (face_non_main_promotion_enabled_
            && comparison_completed && recognition_result.user_name == "unknown") {
            promote_reliable_face_non_main(box);
        }
        if (kMatchTrace) {
            const FaceInferenceCacheEntry& traced = face_inference_cache_.back();
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[FACE_RESULT] f=%d result=infer box=(%d,%d,%d,%d)"
                " q=%.2f sim=%.2f name=%s completed=%d evidence=%s reliableNonMain=%d reason=%s",
                frame_count, box.x, box.y, box.width, box.height,
                face_q, recognition_result.similarity,
                recognition_result.user_name.c_str(),
                comparison_completed ? 1 : 0,
                identity_evidence_name(traced.evidence),
                traced.reliable_non_main ? 1 : 0, traced.reason.c_str());
            trace_push(line);
            std::snprintf(line, sizeof(line),
                "[FACE_EVIDENCE] f=%d box=(%d,%d,%d,%d) status=%s reason=%s",
                frame_count, box.x, box.y, box.width, box.height,
                identity_evidence_name(traced.evidence), traced.reason.c_str());
            trace_push(line);
        }
    ...
    bool standalone_identity_only)
{
    ...
    // identity_only 表示外部 PTZ 已实际改变画面坐标系：此时仍可在当前帧
    // 全体人脸中做身份恢复，但不能再按旧参考点做空间门或排序。
    const bool allow_standalone = standalone_budget > 0
                               && face_registered_
                               && face_recognizer.has_face_template()
                               && last_main_bw_ > 0.f
                               && last_main_bh_ > 0.f
                               && !recovery_faces_.empty()
                               && (standalone_identity_only || standalone_gate > 0.f);
    ...
    if (allow_standalone) {
        ...
        for (int i = 0; i < (int)recovery_faces_.size(); ++i) {
            ...
            float fcy = ((float)f.y + (float)f.height) * 0.5f;
            float dist = 0.f;
            if (!standalone_identity_only) {
                float dx = fcx - standalone_ref->x;
                float dy = fcy - standalone_ref->y;
                dist = std::sqrt(dx * dx + dy * dy);
            }
            float alt_dist = FLT_MAX;
            if (!standalone_identity_only && standalone_alt_ref) {
                const float adx = fcx - standalone_alt_ref->x;
                const float ady = fcy - standalone_alt_ref->y;
                alt_dist = std::sqrt(adx * adx + ady * ady);
            }
            ...
            for (auto& h : face_recovery_hypotheses_) {
                if (standalone_identity_only) break;
            ...
            if (best_hyp == nullptr && !standalone_identity_only) {
            ...
            pool.push_back({
                i, dist, alt_dist,
                !standalone_identity_only
                    && dist > standalone_gate && alt_dist > standalone_gate,
                best_hyp != nullptr ? best_hyp->id : -1,
                best_hyp != nullptr ? best_hyp->last_inferred_frame : -1000000
            });
        }
        std::sort(pool.begin(), pool.end(),
                  [](const StandaloneCand& a, const StandaloneCand& b) {
                      ...
                      return std::min(a.dist, a.alt_dist)
                           < std::min(b.dist, b.alt_dist);
                  });
        ...
        if (take > 0 && !pool.empty() && standalone_identity_only) {
            // PTZ 坐标系失效时只做当前帧公平轮转：不能用旧脸位置、旧 pending
            // 或跨帧空间假设优先任何一张脸。
    ...
    const cv::Mat& img)
{
    ...
    const bool normal_body_global_search = !proximity.all_candidates.empty()
                                 && (in_danger || id_switch_alert_
                                     ...
                                     || body_crowded_frame);
    // 实际 PTZ 滑动后不再把旧 KF/lead 变成全图候选排序依据。保留最后
    // 可靠 BODY 锚点附近的局部候选，另注入恰好一个全局轮转探索候选。
        ...
        auto in_local = [&](const cv::Mat& candidate) {
            ...
            for (int r = 0; r < near_box.rows; ++r)
                if ((int)near_box.at<float>(r, 4) == src
                    && (int)near_box.at<float>(r, 5) == idx) return true;
            return false;
        };
        const int total = proximity.all_candidates.rows;
        for (int step = 0; step < total; ++step) {
            const int row = (ptz_blind_explore_rotor_ + step) % total;
            cv::Mat candidate = proximity.all_candidates.row(row);
            if (in_local(candidate)) continue;
            near_box.push_back(candidate);
            ptz_global_explore_row = near_box.rows - 1;
            ptz_blind_explore_rotor_ = (row + 1) % total;
            break;
        }
        if (kMatchTrace) {
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[PTZ_RECOVERY] f=%d phase=%d anchor_local=%d global_slot=%d total=%d",
                frame_count, (int)ptz_blind_phase_, proximity.match_candidates.rows,
                ptz_global_explore_row, total);
            trace_push(line);
        }
    }
    if (ptz_identity_recovery && kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[SPATIAL_GATE] f=%d disabled_by_ptz_slide=1 iou=0 head=0 motion=0"
            " local_anchor=%d",
    ...
    // FINAL_BODY_OBSERVATION 已改为每个可靠 BODY 帧尝试一次 Pose；这里保留的
    // N 帧节奏仅用于维持 secondary ReID 原有的空闲帧许可，不再触发 winner Pose。
    // 节拍本身必须在 due frame 消费，与本帧 Pose/reference 是否成功无关；否则
    // Pose 连续失败会永久压制 secondary ReID。
    const bool pose_maintenance_due = occlusion_state_ == OcclusionState::CLEAR
                                  && (last_pose_maintenance_frame_ < 0
                                      || frame_count - last_pose_maintenance_frame_
                                             >= kPoseInferEveryN);
    if (pose_maintenance_due)
        last_pose_maintenance_frame_ = frame_count;
    if (pose_maintenance_due && kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[POSE_MAINT] f=%d action=consume_secondary_idle_slot cadence=%d",
            frame_count, kPoseInferEveryN);
        trace_push(line);
    }
    ...
    frame_allow_secondary_reid_ = !in_danger && !body_global_search && !face_priority_frame_
                              && !pose_maintenance_due && !periodic_face_due;
    ...
    std::vector<int> body_ring_for_row(near_box.rows, 0);
    {
        ...
        float rcx, rcy;
        float recovery_cx = -1.f, recovery_cy = -1.f;
        const bool have_recovery_reference = recovery_search_center(
            recovery_cx, recovery_cy);
        ...
        } else if (have_recovery_reference) {
            rcx = recovery_cx; rcy = recovery_cy;
        } else {
        ...
        if (ptz_identity_recovery) {
            ...
            local_rcx = recovery_cx; local_rcy = recovery_cy;
        }
        float coast_cx = -1.f, coast_cy = -1.f;
        int64_t coast_age_ms = -1;
        const bool have_coast_hint = !ptz_identity_recovery && coast_search_hint_center(
            coast_cx, coast_cy, &coast_age_ms);
        for (int k = 0; k < near_box.rows; ++k) {
            ...
            const float lead_dist = std::hypot(dcx - local_rcx, dcy - local_rcy);
            const float coast_dist = have_coast_hint
                ? std::hypot(dcx - coast_cx, dcy - coast_cy) : FLT_MAX;
            // 预测只提供第二个局部搜索圆心。保留 trusted lead/KF 原路径，
            // 不把 PRED 位置变成身份或全局探索参考。
            const float local_dist = std::min(lead_dist, coast_dist);
        ...
        // PTZ blind 的全局候选最多占一个既有 ReID slot。它保证每帧仍有
        // 一次公平探索，但绝不把旧位置无关的全图候选全部送入本帧预算。
        if (ptz_identity_recovery && ptz_global_explore_row >= 0)
            add_selected(ptz_global_explore_row, true);

        if (pending_active_ && !ptz_identity_recovery) {
            int pending_row = -1;
        ...
        if (kMatchTrace) {
            ...
            std::snprintf(line, sizeof(line),
                ...
                " epoch=%d center=(%.0f,%.0f) coast=%d:(%.0f,%.0f)/%lldms"
                " scale=%.0f budget=%d",
                ...
                body_reid_scan_epoch_, rcx, rcy,
                have_coast_hint ? 1 : 0, coast_cx, coast_cy,
                (long long)coast_age_ms, radial_scale, k_budget);
            trace_push(line);
            if (have_coast_hint) {
                std::snprintf(line, sizeof(line),
                    "[RECOVERY_SEARCH_REF] f=%d use=local_reid primary=trusted"
                    " secondary=coast_hint center=(%.1f,%.1f) age=%lldms"
                    " permission=search_rank_only",
                    frame_count, coast_cx, coast_cy,
                    (long long)coast_age_ms);
                trace_push(line);
            }
            ...
            if (ptz_identity_recovery) {
                std::snprintf(line, sizeof(line),
                    "[IDENTITY_REID_SCAN] f=%d local=%d global_slot=%d selected=%d"
                    " budget=%d anchor_only=1 picks=%s",
                    frame_count, proximity.match_candidates.rows,
                    ptz_global_explore_row >= 0 ? 1 : 0,
                    (int)selected.size(), k_budget, picks);
                trace_push(line);
            }
        }
    }
    ...
    // ──────────────────────────────────────────────────────
    // Face 排除预调度：此处的 preliminary rank 只决定把有限 Face slot 给谁，
    // 不生成 winner/runner-up/ambiguity，也不写 KF 或 feature。普通路径至少保留
    // 一个 slot 给后续 A 确认/face-only recovery，因此只在竞争且预算>=2 时探测。
    // ──────────────────────────────────────────────────────
    {
        std::vector<int> preliminary;
        for (int ci = 0; ci < (int)candidates.size(); ++ci)
            if (candidates[ci].reid_evaluated) preliminary.push_back(ci);
        std::sort(preliminary.begin(), preliminary.end(), [&](int a, int b) {
            const Candidate& ca = candidates[a];
            const Candidate& cb = candidates[b];
            return std::max(ca.reid_sim, ca.anchor_sim)
                 > std::max(cb.reid_sim, cb.anchor_sim);
        });
        bool preliminary_competition = false;
        if (preliminary.size() >= 2) {
            const Candidate& a = candidates[preliminary[0]];
            const Candidate& b = candidates[preliminary[1]];
            preliminary_competition =
                (a.reid_sim > kReidAmbiguousMin && b.reid_sim > kReidAmbiguousMin
                 && std::fabs(a.reid_sim - b.reid_sim) < kReidAmbiguousGap)
                || (a.anchor_sim > kAnchorAmbiguousMin && b.anchor_sim > kAnchorAmbiguousMin
                    && std::fabs(a.anchor_sim - b.anchor_sim) < kAnchorAmbiguousGap);
        }
        auto has_body_face = [&](const Candidate& c) {
            const auto& faces = c.source == 0 ? det_groups.dets_one_face
                                               : det_groups.dets_second_face;
            return c.det_index >= 0 && c.det_index < (int)faces.size()
                && !faces[c.det_index].empty() && faces[c.det_index].rows > 0;
        };
        const bool face_exclusion_available = face_registered_
                                           && face_recognizer.has_face_template();
        if (preliminary_competition && face_model_budget_ >= 2
            && face_exclusion_available) {
            int probe_ci = -1;
            // runner-up 是最危险竞争者；没有可用脸时才回退 preliminary winner。
            for (int pi : {1, 0}) {
                if (pi < (int)preliminary.size()
                    && has_body_face(candidates[preliminary[pi]])) {
                    probe_ci = preliminary[pi];
                    break;
                }
            }
            if (probe_ci >= 0) {
                const Candidate& probe = candidates[probe_ci];
                if (kMatchTrace) {
                    char line[kTraceLineLen];
                    std::snprintf(line, sizeof(line),
                        "[FACE_EXCLUSION_SCHEDULE] f=%d src=%d idx=%d r=%.2f A=%.2f"
                        " preliminary_rank=%d budget_before=%d",
                        frame_count, probe.source, probe.det_index,
                        probe.reid_sim, probe.anchor_sim,
                        probe_ci == preliminary[0] ? 1 : 2, face_model_budget_);
                    trace_push(line);
                }
                cv::Mat one = near_box.row(probe.near_idx).clone();
                face_non_main_promotion_enabled_ = true;
                face_recognition_verification(
                    one, 1, det_groups, 0, img,
                    cv::Mat::zeros(0, 2, CV_32S),
                    cv::Mat::zeros(0, 2, CV_32S));
                face_non_main_promotion_enabled_ = false;
            } else if (kMatchTrace) {
                char line[kTraceLineLen];
                std::snprintf(line, sizeof(line),
                    "[FACE_EXCLUSION_SCHEDULE] f=%d action=skip reason=no_owner_face budget=%d",
                    frame_count, face_model_budget_);
                trace_push(line);
            }
        } else if (preliminary_competition && kMatchTrace) {
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[FACE_EXCLUSION_SCHEDULE] f=%d action=skip reason=%s budget=%d",
                frame_count,
                !face_exclusion_available ? "no_main_face_template" : "reserve_budget",
                face_model_budget_);
            trace_push(line);
        }
        ...
        candidates.erase(std::remove_if(candidates.begin(), candidates.end(),
            [&](const Candidate& c) {
                std::string reason;
                const IdentityEvidence evidence = body_identity_evidence_for_box(
                    near_box.row(c.near_idx).colRange(0, 4), &reason);
                if (evidence != IdentityEvidence::NEGATIVE
                    || reason != "reliable_face_non_main") return false;
                if (kMatchTrace) {
                    char line[kTraceLineLen];
                    std::snprintf(line, sizeof(line),
                        "[IDENTITY_SET] f=%d action=exclude_non_main src=%d idx=%d"
                        " hyp=%d r=%.2f A=%.2f reason=%s",
                        frame_count, c.source, c.det_index, c.body_hyp_id,
                        c.reid_sim, c.anchor_sim, reason.c_str());
                    trace_push(line);
                    trace_event_pending_ = true;
                }
                return true;
            }), candidates.end());
        if (kMatchTrace && (before != candidates.size() || preliminary_competition)) {
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[IDENTITY_SET] f=%d final_candidates=%zu excluded=%zu scene_candidates=%d",
                frame_count, candidates.size(), before - candidates.size(), near_box.rows);
            trace_push(line);
        }
    }

    // ──────────────────────────────────────────────────────
    // 自适应融合权重（#2）：外观歧义检测。此处只消费 final identity-valid set；
    // 被可靠确认非 A 的真实人物仍留在 near_box/scene/OCC/coexist 计算中。
    ...
    for (auto& c : candidates) {
        ...
        if (ptz_identity_recovery) {
            // PTZ 已改变画面坐标系：BODY 身份只由当前 ReID/anchor 决定。
            // 旧 IoU、头、遮挡者、Pose/trajectory 均不再参与分数。
    ...
    auto has_strong_reid_direct = [&](const Candidate& candidate) -> bool {
        ...
        if ((body_global_search || ptz_identity_recovery)
            && candidates.size() > 1 && evaluated_count < 2
    ...
    //   danger → 收紧身份门槛 + 遮挡者排斥 + 歧义检测
    {
        ...
        //   参考中心：主目标丢失期用统一 search-only 恢复参考，否则用 KF 中心。
        float mv_ref_cx, mv_ref_cy;
        int   main_tsu_mv = trackers[main_trk_idx]->get_time_since_update();
        const bool have_motion_search_ref = main_tsu_mv > 0
            && recovery_search_center(mv_ref_cx, mv_ref_cy);
        if (!have_motion_search_ref) {
        ...
        for (size_t ci2 = 0; ci2 < candidates.size(); ++ci2) {
            ...
            for (int sti = 0; sti < (int)trackers.size(); ++sti) {
                ...
                if (full_confirm && pos_iou > kCoexistVetoIou) {
                    if (gmc_enabled_) {
        ...
        bool sep_mode = (occlusion_state_ == OcclusionState::RECOVERING
                         || (occlusion_state_ == OcclusionState::OCCLUDED && overlap_count <= 1))
                        && candidates.size() >= 2;
        float sep_ref_cx = (trk_box.at<float>(0, 0)
                          + trk_box.at<float>(0, 2)) * 0.5f;
        float sep_ref_cy = (trk_box.at<float>(0, 1)
                          + trk_box.at<float>(0, 3)) * 0.5f;
        if (emergence_valid_ && now_match_ms >= emergence_update_ms_
            && now_match_ms - emergence_update_ms_ <= kEmergenceMaxAgeMs) {
            sep_ref_cx = emergence_cx_;
            sep_ref_cy = emergence_cy_;
        }
        ...
        if (!gmc_enabled_ && reid_ambiguous && mv_min_dist < FLT_MAX) {
            for (size_t ci2 = 0; ci2 < candidates.size(); ++ci2) {
                if (!candidates[ci2].reid_evaluated
        ...
        for (auto& c : candidates) {
            ...
            if (!ptz_identity_recovery && sep_mode) {
                ...
                if (best_traj >= 0 && &candidates[best_traj] != &c) {
                    ...
                    if (traj_gap > kSepTrajMargin && app_lead < kSepAppDominant
                        ...
                        break;
                    } else if (traj_gap > kSepTrajMargin && strong_sep_direct) {
            ...
            const bool candidate_spatial_conflict = !ptz_identity_recovery
                && !gmc_enabled_ && (coexist_spatial || trajectory_spatial);
            if (candidate_spatial_conflict) {
    ...
    if (best.matched && occlusion_state_ == OcclusionState::OCCLUDED && !best.from_face) {
        ...
        if ((best.source == 0 || best.source == 1)
            ...
            commit_iou = ptz_identity_recovery ? 0.f
                                                : Utils::iou_single(trk_box, commit_box);
            has_commit_box = true;
        }
    ...
    bool body_needs_face_arbiter = best.matched
                                && !best.from_face
                                && global_face_watch;
    ...
    if (best.matched) {
        ...
        if (!ptz_identity_recovery && !long_blind && has_bb) {
            float rcx, rcy;
        ...
        bool head_discont = !ptz_identity_recovery && head_track_fresh && head_tsu <= kHeadVetoMaxAge
        ...
        if (long_blind || teleport || head_discont || head_only_long || sweep_face_hyp) {
            ...
            bool same_hyp = !ptz_identity_recovery && pending_active_ && has_bb
            ...
            if (has_bb && !ptz_identity_recovery) {
    ...
    if (best.matched) {
        ...
        if (reliable_measurement) {
            ...
            if ((best.source == 0 || best.source == 1)
                ...
                const cv::Mat reliable_box =
                    positive_src.row(best.index).colRange(0, 4);
                const bool ptz_rebase_this_measurement = spatial_prior_invalid_;
                if (ptz_rebase_this_measurement) {
                    finish_ptz_blind_reacquisition(main_trk_idx, reliable_box);
                } else if (ptz_reacq_body_streak_ > 0) {
                    ++ptz_reacq_body_streak_;
                }
                update_ptz_blind_anchor(reliable_box, now_match_ms);
            }
            if (kMatchTrace) {
                char line[kTraceLineLen];
                std::snprintf(line, sizeof(line),
                    "[MEASUREMENT] f=%d class=RELIABLE src=%d idx=%d"
                    " risk=%d personRisk=%d strong=%d head=%d r=%.2f A=%.2f"
                    " kf_update=1 timer_reset=1",
                    frame_count, best.source, best.index, identity_risk ? 1 : 0,
                ...
                // if (ptz_rebase_this_measurement) {
                //     std::snprintf(line, sizeof(line),
                //         "[IDENTITY_COMMIT] f=%d source=BODY src=%d idx=%d"
                //         " r=%.2f A=%.2f reason=%s feature_update=freeze",
                //         frame_count, best.source, best.index,
                //         best.reid_sim, best.anchor_sim,
                //         strong_identity ? "strong_identity" : "reliable_body");
                //     trace_push(line);
            ...
            if (kMatchTrace) {
                ...
                std::snprintf(line, sizeof(line),
                    ...
                    " reason=%s personRisk=%d r=%.2f A=%.2f"
                    " kf_update=0 feature_update=0 timer_reset=0",
                    frame_count, uncertain_src, uncertain_idx,
                    person_identity_risk_active
        ...
        if (kMatchTrace) {
            ...
            std::snprintf(line, sizeof(line),
                ...
                " candidates=%d pending=%d facePending=%d"
                " kf_update=0 feature_update=0 timer_reset=0",
    ...
    if (kMatchTrace) {
        ...
        std::snprintf(_ml, sizeof(_ml),
            ...
            " | matched=%d meas=%s personRisk=%d src=%d idx=%d"
            ...
            best.source, best.index, (best.from_face ? 1 : 0),
    ...
    // ──────────────────────────────────────────────────────
    if (best.matched && !best.from_face
        && !(ptz_reacq_body_streak_ > 0 && ptz_reacq_body_streak_ < 2)
        && visibility_state_ == VisibilityState::FULL
    ...
    // 所有身份、共存、OCC、provisional 和人脸门均已完成后，只有最终可靠 BODY
    // 才能获得 FINAL_BODY_OBSERVATION。歧义阶段已有当前帧 Pose 直接复用，否则
    // 使用为最终 winner 保留的独立 slot；不会为 tentative/UNCERTAIN 候选买 Pose。
    if (best.matched && (best.source == 0 || best.source == 1)) {
        ...
        if (best.index >= 0 && best.index < pose_src.rows) {
            ...
            if (best.reliability == MeasurementReliability::RELIABLE) {
                pose_entry = request_pose(img, winner_box, best.source, best.index,
                                          PoseReason::FINAL_BODY_OBSERVATION);
                observe_final_body_pose(pose_entry, winner_box, img,
                                        best.source, best.index);
            }
    ...
    if (kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[POSE_FRAME] f=%d total=%d cand=%d final=%d cache=%zu state=%d winner=%d/%d matched=%d obs=%d ref=%d",
            frame_count, pose_budget_used_, pose_candidate_budget_used_, pose_final_budget_used_,
            pose_cache_.size(),
            (int)occlusion_state_, best.source, best.index, best.matched ? 1 : 0,
            current_pose_observation_.valid ? 1 : 0,
            current_pose_observation_.reference_committed ? 1 : 0);
        trace_push(line);
    }
...
bool LightTracker::should_update_embedding(
    ...
    int close_det_count)
{
    // PTZ 坐标系切换后的首个可靠 BODY 只重建空间状态，不能立即用一次
    // re-acquisition 特征污染主模板；第二个可靠 BODY 后恢复原有更新门。
    if (ptz_reacq_body_streak_ > 0 && ptz_reacq_body_streak_ < 2)
        return false;
    ...
    const cv::Mat& dets_second)
{
    ...
    cv::Mat main_box = main_trk.row(0);
    const bool ptz_identity_recovery = spatial_prior_invalid_
        && !ptz_blind_anchor_box_.empty();
    if (ptz_identity_recovery)
        main_box = ptz_blind_anchor_box_;
    ...
    float gate_ref_cx = -1.f, gate_ref_cy = -1.f;
    if (!ptz_identity_recovery && frames_lost > 0
        && recovery_search_center(gate_ref_cx, gate_ref_cy)) {
        main_cx = gate_ref_cx;
        main_cy = gate_ref_cy;
        null_sink << "[REACQ] search-gate"
                  << " center=(" << main_cx << "," << main_cy
                  << ") frames_lost=" << frames_lost << std::endl;
    }

    float coast_cx = -1.f, coast_cy = -1.f;
    int64_t coast_age_ms = -1;
    const bool have_coast_hint = !ptz_identity_recovery && frames_lost > 0
        && coast_search_hint_center(coast_cx, coast_cy, &coast_age_ms);
    ...
    struct DetInfo {
        cv::Mat box;   // 4列
        float   dist;        // trusted lead/emergence/KF 距离，保持 close 统计语义
        float   coast_dist;  // PRED search-only hint 距离，不参与 close/overlap
        int     source;
        int     index;
    };
    ...
    auto collect = [&](const cv::Mat& dets, int source_id) {
        for (int i = 0; i < dets.rows; ++i) {
            ...
            const float coast_dist = have_coast_hint
                ? std::hypot(dcx - coast_cx, dcy - coast_cy) : FLT_MAX;
            all_dets.push_back({det.clone(), dist, coast_dist, source_id, i});
        }
    };
    ...
    // PTZ 实际滑动后，最后可靠 BODY 框只作本地预选锚点；它不参与任何
    // KF/lead/coast 双中心排序。所有实际身份门仍在 match_main_target_unified。
    ...
    for (auto& d : all_dets) {
        // 正常路径保留历史 coast 双中心。PTZ 坐标系失效期只使用最后真实 BODY
        // 搜索锚点，避免过期 PRED/KF 位置抢占本地候选。
        if (d.dist <= match_radius || d.coast_dist <= match_radius)
            match_pool.push_back(d);
    }
    std::sort(match_pool.begin(), match_pool.end(),
              [ptz_identity_recovery](const DetInfo& a, const DetInfo& b) {
                  const float ka = ptz_identity_recovery ? a.dist
                                                         : std::min(a.dist, a.coast_dist);
                  const float kb = ptz_identity_recovery ? b.dist
                                                         : std::min(b.dist, b.coast_dist);
                  return ka < kb;
              });
    ...
    if (have_coast_hint && kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[RECOVERY_SEARCH_REF] f=%d use=match_gate primary=trusted"
            " secondary=coast_hint center=(%.1f,%.1f) age=%lldms"
            " radius=%.1f permission=search_rank_only",
            frame_count, coast_cx, coast_cy, (long long)coast_age_ms,
            match_radius);
        trace_push(line);
    }
...
LightTracker::PoseCacheEntry* LightTracker::request_pose(
    ...
    PoseReason reason) {
    const bool final_observation = reason == PoseReason::FINAL_BODY_OBSERVATION;
    const char* reason_name = final_observation ? "FINAL_BODY" : "AMBIG";
    PoseCacheEntry* cached = find_cached_pose(source, index);
    // 候选阶段可能因其自身预算耗尽留下 BUDGET_DENIED cache；最终可靠 winner
    // 必须仍可使用独立 final slot，故仅在该特例下允许复用条目后继续执行。
    if (cached && !(final_observation
                    && cached->status == PoseRequestStatus::BUDGET_DENIED)) {
        if (kMatchTrace) {
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[POSE_REQ] f=%d reason=%s src=%d idx=%d status=CACHE cached=%d total=%d cand=%d final=%d",
                frame_count, reason_name, source, index, (int)cached->status,
                pose_budget_used_, pose_candidate_budget_used_, pose_final_budget_used_);
            trace_push(line);
        }
        return cached;
    }
    if (cached == nullptr) {
        ...
        cached = &pose_cache_.back();
    }
    const bool budget_denied = final_observation
        ? pose_final_budget_used_ >= kPoseFinalBudgetPerFrame
        : pose_candidate_budget_used_ >= kPoseCandidateBudgetPerFrame;
    if (budget_denied) {
        cached->status = PoseRequestStatus::BUDGET_DENIED;
        if (kMatchTrace) {
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[POSE_REQ] f=%d reason=%s src=%d idx=%d status=BUDGET total=%d cand=%d/%d final=%d/%d",
                frame_count, reason_name, source, index, pose_budget_used_,
                pose_candidate_budget_used_, kPoseCandidateBudgetPerFrame,
                pose_final_budget_used_, kPoseFinalBudgetPerFrame);
            trace_push(line);
        }
    ...
    if (final_observation) ++pose_final_budget_used_;
    else ++pose_candidate_budget_used_;
    ++pose_budget_used_;
    const int64_t start_ms = now_ms();
    const int ret = pose_estimator.run(img, body_box, cached->pose);
    cached->status = ret == 0 ? PoseRequestStatus::AVAILABLE : PoseRequestStatus::FAILED;
    if (kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[POSE_RUN] f=%d reason=%s src=%d idx=%d status=%s total=%d cand=%d final=%d latency=%lldms kp=%d",
            frame_count, reason_name, source, index, ret == 0 ? "OK" : "FAIL",
            pose_budget_used_, pose_candidate_budget_used_, pose_final_budget_used_,
            (long long)(now_ms() - start_ms), cached->pose.valid_kp_count);
        trace_push(line);
    }
    return cached;
}

void LightTracker::observe_final_body_pose(const PoseCacheEntry* pose_entry,
                                           const cv::Mat& body_box,
                                           const cv::Mat& img,
                                           int source, int index) {
    current_pose_observation_ = CurrentPoseObservation{};
    current_pose_observation_.source = source;
    current_pose_observation_.index = index;
    current_pose_observation_.timestamp_ms = now_ms();
    current_pose_observation_.body_box = body_box.clone();

    const bool pose_available = pose_entry != nullptr
                             && pose_entry->status == PoseRequestStatus::AVAILABLE;
    if (!pose_available) {
        // 可靠 BODY 仍存在：可见度使用当前框的几何退化，朝向当前帧降为 UNKNOWN；
        // 最近可靠 Pose reference 保留给下一帧 OKS，不能被失败 observation 擦除。
        assess_visibility(body_box, nullptr, img.cols, img.rows);
        if (kMatchTrace) {
            char line[kTraceLineLen];
            std::snprintf(line, sizeof(line),
                "[POSE_OBSERVATION] f=%d src=%d idx=%d valid=0 reason=%s vis=%d current_frontal=-1.00 ref_frontal=%.2f",
                frame_count, source, index,
                pose_entry == nullptr ? "NOT_REQUESTED" : "POSE_UNAVAILABLE",
                (int)visibility_state_, frontalness_);
            trace_push(line);
        }
        return;
    }

    current_pose_observation_.valid = true;
    current_pose_observation_.pose = pose_entry->pose;
    current_pose_observation_.proportions = PoseMatch::extract_body_proportions(
        pose_entry->pose.keypoints, 0.4f);
    if (current_pose_observation_.proportions.valid) {
        for (auto& trk : trackers) {
            if (trk->get_is_main()) {
                trk->set_body_proportions(current_pose_observation_.proportions);
                break;
            }
        }
    }
    assess_visibility(body_box, &pose_entry->pose, img.cols, img.rows);

    // current orientation 与跨帧 orientation reference 分开：本帧 Pose 不足或
    // PTZ 坐标系失效时只把 current 标为 UNKNOWN，不能擦除既有可靠参考，更不能
    // 在 spatial-invalid epoch 写入会参与下一帧身份评分的几何/朝向状态。
    cv::Point2f shoulder_mid;
    float shoulder_width = 0.f;
    const bool orientation_eligible = !spatial_prior_invalid_
                                   && body_box.cols >= 4
                                   && (body_box.at<float>(0, 3)
                                       - body_box.at<float>(0, 1)) >= kOrientMinTorsoPx
                                   && (body_box.at<float>(0, 2)
                                       - body_box.at<float>(0, 0)) >= 4.f
                                   && extract_shoulder_geom(pose_entry->pose.keypoints,
                                                            0.4f, shoulder_mid,
                                                            shoulder_width);
    if (orientation_eligible) {
        update_orientation_state(&pose_entry->pose, body_box);
        current_pose_observation_.orientation = frontalness_;
    }

    // 当前 observation 可不完整；跨帧 OKS reference 仍只接受足够结构点、当前
    // 真实可靠 BODY 且未处于 PTZ 旧坐标系失效 epoch 的结果。
    const bool reference_eligible = pose_entry->pose.valid_kp_count >= 5
                                 && !spatial_prior_invalid_;
    const char* reference_gate = spatial_prior_invalid_ ? "SPATIAL_INVALID"
                              : pose_entry->pose.valid_kp_count < 5 ? "KP_LT_5"
                              : "COMMIT";
    if (reference_eligible) {
        commit_pose(pose_entry->pose, body_box, img, "final_body_reference");
        current_pose_observation_.reference_committed = true;
    }
    if (kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[POSE_OBSERVATION] f=%d src=%d idx=%d valid=1 kp=%d ref=%d ref_gate=%s orient_gate=%s vis=%d current_frontal=%.2f ref_frontal=%.2f",
            frame_count, source, index, pose_entry->pose.valid_kp_count,
            current_pose_observation_.reference_committed ? 1 : 0,
            reference_gate, orientation_eligible ? "UPDATE" : "UNKNOWN",
            (int)visibility_state_, current_pose_observation_.orientation,
            frontalness_);
        trace_push(line);
    }
}
...
void LightTracker::commit_pose(const PoseResult& pose, const cv::Mat& body_box,
    ...
    if (kMatchTrace) {
        char line[kTraceLineLen];
        std::snprintf(line, sizeof(line),
            "[POSE_REFERENCE] f=%d reason=%s kp=%d",
...
void LightTracker::reset() {
    ...
    // 重置候选级 Pose cache、逐帧 observation、跨帧 reference 与 PTZ-only trunk 状态。
    pose_cache_.clear();
    pose_budget_used_ = 0;
    pose_candidate_budget_used_ = 0;
    pose_final_budget_used_ = 0;
    ...
    last_pose_maintenance_frame_ = -1;
    reset_ptz_body_output_transition();
