...
//  [N10] 自运动前馈（无 GMC 的替代先验）：仅在下游确实把输出拉向画面中心、且其响应
//        可近似为固定 β 时，用 -β·e 预测表观位移，平移 lead/smooth/emergence 参考点
//        并放宽相关门预算；只动"软参考点"，绝不碰 KF 状态。实际 PTZ 若有死区、延迟、
//        位置相关速度或非中心构图目标，则该近似不成立，保持默认关闭。
...
private:
    ...
    AclRuntime acl_runtime_;
    CFaceKeypoint106 detector_fk;
    FaceRecognitionSystem face_recognizer;
    CPersonReID emb_model;
    Detector_yolox detector;
    PoseEstimator pose_estimator;
    bool initialized_ = false;

    void release_models();

    // Pose 请求分为候选身份仲裁与最终可靠 BODY 的当前帧观测。
    // 后者同时服务可见度、朝向、可靠 Pose reference 与 PTZ trunk，不能被
    // ambiguity 候选预算挤掉。
    enum class PoseReason { AMBIGUITY_RESOLVE, FINAL_BODY_OBSERVATION };
    ...
    int pose_budget_used_ = 0;            // 当帧实际 Pose 总调用数（trace/诊断）
    int pose_candidate_budget_used_ = 0;  // ambiguity candidate 已消费次数
    int pose_final_budget_used_ = 0;      // final BODY 保留 slot 已消费次数
    static constexpr int kPoseCandidateBudgetPerFrame = 2;
    static constexpr int kPoseFinalBudgetPerFrame = 1;
    // 仅当前帧有效：最终可靠 BODY 的真实 Pose observation。它可服务当前帧
    // visibility/orientation/trunk；是否进入跨帧 reference 由独立质量门决定。
    struct CurrentPoseObservation {
        bool valid = false;
        bool reference_committed = false;
        // 当前帧朝向；-1 表示本帧 UNKNOWN。它不清除 frontalness_ 中保留的
        // 跨帧可靠朝向参考。
        ...
        cv::Mat body_box;
        PoseResult pose{};
        BodyProportionDescriptor proportions{};
    };
    CurrentPoseObservation current_pose_observation_;
    // 以下仅表示跨帧可靠 Pose reference，供下一帧 OKS/肩部连续性使用；不是
    // 每个 current observation 都有资格覆盖它。
    int last_committed_pose_frame_ = -1;
    int64_t last_committed_pose_ms_ = -1;
    cv::Mat committed_pose_box_;
    // 保留原每 N 帧的 secondary ReID 节奏，不能因为 final BODY Pose 改为逐帧
    // 观测就隐式改变 secondary ReID 的许可频率。这个节拍与 Pose 成功与否无关。
    int last_pose_maintenance_frame_ = -1;
    ...
    // 历史 Pose refresh 节拍现只保留为 secondary ReID 的空闲帧节拍；最终可靠
    // BODY Pose 不受它限制，每帧均可使用 FINAL_BODY_OBSERVATION。
    ...
    // ── 10 FPS 帧级模型预算 ──
    // 人脸与候选 Pose 分别有独立硬预算：ambiguity candidate 最多两次，最终
    // 可靠 BODY 另有一个保留 slot；同一候选的同帧结果会复用。
    ...
    // 只在 preliminary Face 排除调度的短作用域内打开。普通 A 确认、face-only
    // recovery 与缓存复用中的 mismatch 必须保持 UNKNOWN，不能迟到地改写 final
    // identity arbitration。
    bool face_non_main_promotion_enabled_ = false;
    ...
    // 非主目标排除的质量门高于普通 A 识别门；宁可少排除，也不能错排 A。
    static constexpr float kFaceReliableNonMainMinQ = 0.80f;
    // 与 A 模板的最大相似度必须显著低于现有 A 正匹配门（0.50），才可在
    // preliminary 专用作用域内升级为“可靠非 A”。两门之间的 (0.40, 0.50)
    // 缓冲带一律保持 UNKNOWN，不能因普通 mismatch 排除主目标。
    static constexpr float kFaceReliableNonMainMaxSimilarity = 0.40f;

    // PTZ 主干框：只使用当前帧已接受 BODY 对应的 COCO17 双肩/手腕；不参与
    // detection、KF、ReID、Face、Motion History 或任何身份判定。
    static constexpr float kPtzTrunkKeypointConf = 0.30f;
    static constexpr float kPtzTrunkShoulderSidePadRatio = 0.35f;
    static constexpr float kPtzTrunkHeadTopRatio = 1.10f;
    static constexpr float kPtzTrunkHandRaiseMarginRatio = 0.05f;
    static constexpr float kPtzTrunkMinShoulderWidthPx = 3.0f;
    ...
    // 外部 PTZ 已实际滑动后的本地搜索锚点。它只由最后一次可靠 BODY 更新；
    ...
    static constexpr int64_t kShortPredictionMaxDurationMs = 400;
    // PRED 耗尽后，最后一个画面内预测位置只作为短时重捕排序提示保留。
    // 它不是身份/量测参考，生命周期不得长于一次短时预测窗口。
    static constexpr int64_t kCoastSearchHintHoldMs = 400;
    static constexpr float kShortPredictionNominalDtMs = 40.0f;
    // MOVE 的位移以冻结 anchor 的 BODY 宽度为主尺度；400 ms 仍只是
    // image-space 预测的新鲜度硬上限，不因目标较小而延长。
    static constexpr float kShortPredictionMaxBodyWidths = 1.0f;
    ...
    struct FrozenPredictionState {
        ...
        // 启动时冻结，ACTIVE 中不得被候选/KF 改写；clear/reset 时随整个
        // FrozenPredictionState 清除，只服务当前 PRED 的距离边界与 trace。
        float max_body_width_displacement = 0.f;
    ...
    // 历史 PRED 的搜索提示。PRED 输出已停用；进入外部 PTZ blind 的 HOLD
    // 路径会清除它，不能改写 trusted lead、ReID anchor、KF、身份特征或
    // 真实观测时钟。
    struct CoastSearchHint {
        bool valid = false;
        float cx = -1.f, cy = -1.f;
        int64_t update_ms = -1;
        int anchor_frame_id = -1;
    ...
    CoastSearchHint coast_search_hint_;
    ...
    // 最终可靠 BODY 的 trunk/raw 输出切换专用状态。它只控制交给 PTZ 的
    // xyxy 框；绝不回写 BODY measurement、KF、Motion History 或 Pose reference。
    struct PtzBodyOutputTransition {
        bool last_trunk_valid = false;
        cv::Mat last_trunk_box;
        int last_trunk_frame = -1;
        bool last_output_valid = false;
        cv::Mat last_output_box;
        int last_output_frame = -1;
    };
    PtzBodyOutputTransition ptz_body_output_transition_;
    static constexpr int kPtzTrunkHoldFrames = 1;
    static constexpr float kPtzTrunkTransitionAlpha = 0.35f;
    ...
    // 仅当前帧有效：唯一 owner 的正式上部居中关联脸才可传播非主目标证据。
    std::vector<char>     recovery_face_owner_exclusion_eligible_;
    ...
    // 跨帧可靠朝向 EMA；current observation 的 UNKNOWN 不得清空它，-1 表示尚无
    // 可安全使用的参考（下游门控空操作）。
    float   frontalness_ = -1.f;
    ...
    static constexpr float   kIncumbentHysteresis = 0.2f;
...
public:
    ...
    std::pair<cv::Mat, int> update(const cv::Mat& img, const cv::Rect& mainBox);
...
private:
    ...
    // 最终 BODY 的 PTZ-only trunk/raw 过渡；raw_body_box 已经过既有输出稳定器。
    cv::Mat apply_ptz_body_output_transition(const cv::Mat& raw_body_box,
                                              const cv::Mat& current_trunk_box);
    void reset_ptz_body_output_transition();
    // 仅修改最终 PTZ BODY 输出框；body_box 与 pose 均来自当前帧已接受的同一
    // BODY candidate。失败由调用方保持原有输出框。
    bool compute_ptz_trunk_box(const cv::Mat& body_box, const PoseResult& pose,
                               cv::Mat& trunk_box);

    // 历史短时 image-space 预测实现保留为内部停用代码；丢失后的 PTZ blind slide
    // 已由外部 PTZ 控制层完成，LightTracker 不再调用它输出 PRED。
    ...
    bool coast_search_hint_center(float& cx, float& cy,
                                  int64_t* age_ms = nullptr) const;
    void update_coast_search_hint(const cv::Mat& predicted_box, int64_t now);
    void clear_coast_search_hint(const char* reason);
    void expire_coast_search_hint(int64_t now);
    ...
    bool face_owner_allows_non_main(int face_idx,
                                    std::string* reason = nullptr) const;
    void promote_reliable_face_non_main(const cv::Rect& face_box);
    ...
    // current observation 与可靠跨帧 reference 分开：前者每个可靠 BODY 帧更新；
    // 后者仅由安全、可用于下一帧身份比较的 Pose 刷新。
    void observe_final_body_pose(const PoseCacheEntry* pose_entry,
                                 const cv::Mat& body_box, const cv::Mat& img,
