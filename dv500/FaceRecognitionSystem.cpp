...
int FaceRecognitionSystem::init(FaceRecogConfigDV500 config,
    float threshold,
    float det_thresh) {

    release();
    face_recognizer_ = std::make_unique<CFaceRecognitionDV500>();
...
FaceRecognitionResult FaceRecognitionSystem::recognition(const cv::Mat& image,
    ...
    if (is_frontal) {
        ...
        embedding = face_recognizer_->get(image, kps);
...
std::string FaceRecognitionSystem::register_face(const cv::Mat& image,
    ...
    std::vector<float> embedding = face_recognizer_->get(image, kps);
    if (embedding.empty()) {
